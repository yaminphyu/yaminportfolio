...
export default function Footer() {
  return (
    <footer className='w-full h-full flex flex-col items-center justify-center gap-3'>
      ...
      <span className='text-white'>© 2025 PrebuiltUI. All rights reserved. Distributed by ThemeWagon</span>
      <div>
        <ul className='flex gap-7'>
          {
            SOCIAL_MEDIA.map((social: any, index: number) => {
              return (
                <li key={index} className="inline-block mr-6 text-white hover:text-gray-200">
                  <Link
                    href={social.url}
                    target="_blank"
